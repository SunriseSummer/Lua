...
- 整数字面量（如 `42`）为 `Int64`；`Float64` 字面量必须包含 `.`（如 `3.`, `1.0`, `3.14`）。
- `Int64` 与 `Float64` 严格区分，赋值或传参跨类型时需显式转换：`Int64(x)`、`Float64(x)`；数字字符串也可转换。
- 混合算术中 `Int64` 自动提升为 `Float64`，表达式结果为 `Float64`；同类型运算结果保持原类型。
