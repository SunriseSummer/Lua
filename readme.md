...
- **索引取值**：`s[0]` 返回第 0 个 UTF-8 字符（Rune 类型，0-based）
- **切片取值**：`s[1..4]`（排他）、`s[0..=2]`（包含）返回子字符串，基于字符位置
- **长度属性**：`s.size` 返回 UTF-8 字符数量（非字节数）
- **isEmpty 属性**：`s.isEmpty` 判断字符串是否为空
- **常用成员方法**（参考仓颉 String 设计）：
  ...
  - `s.split(sep)` — 按分隔符拆分，返回 0-based 数组；传空串按 UTF-8 字符拆分
  ...
  - `s.toRuneArray()` — 转为 `Array<Rune>`（UTF-8 字符数组）

```cangjie
let s = "Hello你好😀"
println(s.size)        // 8（字符数，非字节数）
println(s[5])          // "你"（返回 Rune 字符）
println(s[5..=6])      // "你好"
println(s.contains("你好"))  // true
println(s.indexOf("你好"))   // 5
println(s.toArray().size)    // 15（UTF-8 字节数）
println(s.toRuneArray().size) // 8（字符数）

// Array<Byte> 与 String 互转
let bytes = s.toArray()       // String -> Array<Byte>
...
// String -> Array<Rune>（UTF-8 字符数组）
let runes = s.toRuneArray()
println(runes.size)    // 7（字符数）
println(runes[5])      // "你"
```
