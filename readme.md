...
- **算术运算**：`+`、`-`、`*`、`/`、`%`、`**`（幂运算）
- **比较运算**：`==`、`!=`、`>`、`<`、`>=`、`<=`
- **逻辑运算**：`&&`（逻辑与）、`||`（逻辑或）、`!`（逻辑非），支持短路求值
- **字符串连接**：`+`（也支持 Lua 风格的 `..`）
- **位运算**：`&`（按位与）、`|`（按位或）、`^`（按位异或）、`~`（按位求反）、`<<`（左移）、`>>`（右移）
...
- **Lambda 表达式**（花括号语法）：
  ...
  - 无类型注解：`{ x, y => x * y }`
  - 无参数：`{ => println("hello") }`
  - 多语句体：`{ x => var y = x + 1; return y }`
- **Lambda 表达式**（圆括号语法）：`() => 42` 或 `() => { statements }`
- **命名参数**：使用 `!` 后缀声明命名参数，支持默认值
  ```cangjie
...
- `llex.c/h` - 词法分析器，定义仓颉关键字和运算符（`&&`、`||`、`!`、`**`、`^`、`&`、`|` 等）
- `lparser.c/h` - 语法分析器，实现仓颉语法到 Lua 字节码的编译（Lambda 表达式、函数类型、命名参数等）
