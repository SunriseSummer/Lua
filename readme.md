...
Rune 是独立的类型，与整型和字符串严格区分。Rune 类型支持关系操作符（`<`、`>`、`<=`、`>=`、`==`、`!=`），**仅**在 Rune 之间比较 Unicode 值；与整型比较时必须显式使用 `Int64(Rune)` 转换。可以通过字符 Unicode 码点值构造对应的 Rune 实例，即 `Rune(code_point)`。

- `Int64(r'A') == 65`（Rune 与整型比较需显式转换）
